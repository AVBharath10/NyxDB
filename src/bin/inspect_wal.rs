...
fn main() -> std::io::Result<()> {
    ...
    loop {
        ...
        match opcode {
            1 => { // PUT
                 if cursor + 4 > data.len() { println!("Corrupt(KeyLen)"); continue; }
                 let key_len = u32::from_le_bytes(data[cursor..cursor+4].try_into().unwrap()) as usize;
                 cursor += 4;

                 if cursor + key_len > data.len() { println!("Corrupt(Key)"); continue; }
                 let key = &data[cursor..cursor+key_len];
                 cursor += key_len;

                 if cursor + 4 > data.len() { println!("Corrupt(ValLen)"); continue; }
                 let val_len = u32::from_le_bytes(data[cursor..cursor+4].try_into().unwrap()) as usize;
                 cursor += 4;

                 if cursor + val_len > data.len() { println!("Corrupt(Value)"); continue; }
                 let value = &data[cursor..cursor+val_len];
                 
                 println!("PUT Key='{}' Value='{}'", String::from_utf8_lossy(key), String::from_utf8_lossy(value));
            }
            2 => { // DELETE
                 if cursor + 4 > data.len() { println!("Corrupt(KeyLen)"); continue; }
                 let key_len = u32::from_le_bytes(data[cursor..cursor+4].try_into().unwrap()) as usize;
